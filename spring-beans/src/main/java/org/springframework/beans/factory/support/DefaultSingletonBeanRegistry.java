...
//	private final Map<String, Object> singletonObjects = new ConcurrentHashMap<>(256);
	...
	/**
	 * 三级缓存
	 * 用于保存BeanName和bean工厂之间的关系
	 * TODO：BeanFactory，FactoryBean和ObjectFactory之间的关系？
...
//	private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<>(16);
	...
	/**
	 * 二级缓存
	 * 包船BeanName和bean之间的关系，与singletonFactories的不同之处在于，当一个单例bean被放到这里之后，那么当bean还在创建过程中
...
//	private final Set<String> registeredSingletons = new LinkedHashSet<>(256);
	...
	/**
	 * 先调用这个getSingleton，判断一级缓存是否含有对象
	 * 如果含有对象，则直接返回
	 * 如果不含有对象并且没有处于正在创建中状态，则返回null，需要调用下面的
	 *
	 * @param beanName            the name of the bean to look for
	 * @param allowEarlyReference whether early references should be created or not
	 * @return the registered singleton object, or {@code null} if none found
	 * @see #getSingleton(String, ObjectFactory)
	 * 如果不包含对象并且没有处于正在创建中状态，因为createBean是对一级缓存加锁的。所以只能是
	 * 当前对象通过填充属性的链条又找到了当前对象，即存在循环引用。使用三级缓存解决
		...
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			...
			if (singletonObject == null && allowEarlyReference) {
				synchronized (this.singletonObjects) {
					...
					// 进入锁之后，一级和二级缓存也有可能发生了变化。所以要做双重检查
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				//将beanName添加到singletonsCurrentlyInCreation，标志当前beanName处于正在创建中。
				//循环依赖会用到该集合singletonsCurrentlyInCreation
				...
				try {
					...
					//将对象加入一级缓存，那么doCreateBean中的方法访问时一级缓存中并没有对应的bean对象
					singletonObject = this.singletonObjects.get(beanName);
					if (singletonObject == null) {
						throw ex;
					}
