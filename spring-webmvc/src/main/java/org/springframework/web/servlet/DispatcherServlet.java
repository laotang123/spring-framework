...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		//FORM1：从容器中获取对象，子容器——》父容器。容器中不存在就返回null
		initMultipartResolver(context);
		//FORM2：从容器中获取对象，容器中不存在创建默认策略(webmvc模块中的DispatcherServlet.properties)
		initLocaleResolver(context);
		//FORM2：创建顺序，容器中获取——》默认创建策略
		initThemeResolver(context);
		//FORM3：获取顺序，容器中查找handlerMapping的子类对象——》获取容器中beanName为handlerMapping的bean对象——》默认的创建策略
		initHandlerMappings(context);
		//FORM3：获取容器中HandlerAdapter的子类对象——》容器中beanName为handlerAdapter的bean对象——》默认的创建策略
		initHandlerAdapters(context);
		//FORM3：获取容器中HandlerExceptionResolver的子类对象——》容器中beanName为handleExceptionResolver的bean对象——》默认的创建策略
		initHandlerExceptionResolvers(context);
		//FORM2：获取容器中beanName为viewNameTranslator的bean对象——》默认的创建策略
		initRequestToViewNameTranslator(context);
		//FORM3：获取容器中ViewResolver的子类对象——》容器中beanName为viewResolver的bean对象——》默认的创建策略
		initViewResolvers(context);
		//FORM2：获取容器中beanName为flashMapManager的bean对象——》默认的创建策略
